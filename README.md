...
DistractMath-BN is a distractor-augmented benchmark derived from the Bangla subset of MGSM (250 problems) and MSVAMP (1000 problems). Each problem includes three distractor types:

| Distractor Type | Description | Example |
|----------------|-------------|---------|
| **RED** (Related Entity) | Numerical info about same object type but different entities | "Her sister has 18 lollipops" |
| **OAD** (Orthogonal Attribute) | Properties in different dimensions than queried | "Each pen costs 15 taka" |
| **NEED** (Null-Effect Event) | Actions with zero net impact (planned but not executed) | "Raju agreed to give 1000, but didn't" |
