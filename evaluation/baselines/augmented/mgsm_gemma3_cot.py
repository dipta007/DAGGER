# ============================================================
# MGSM Bangla Evaluation with Few-shot CoT from https://arxiv.org/pdf/2210.03057 (native)
# - checkpointing
# - token usage stats
# ============================================================
...
def run_mgsm_bn(
    split="test",
    max_samples=None,
    checkpoint="mgsm_aug_gemma3_4b_checkpoint.json",
    out_json="mgsm_aug_gemma3_4b_detailed.json",
    out_csv="mgsm_aug_gemma3_4b_summary.csv",
    max_new_tokens=2048
):
    ...
    ds = pd.read_csv("augmented_mgsm.csv")
    ...
    for i, (_, ex) in enumerate(tqdm(ds.iterrows(), total=len(ds))):
        if i in done:
            continue

        prompt = build_prompt(ex["modified_question"])
        ...
        row = {
            "id": i,
            "question": ex["modified_question"],
            "gold": gold,
            "pred": pred,
            "aug_type": ex["augmentation_type"],
            "correct": correct,
            "input_tokens": in_tok,
    ...
    df[["id", "gold", "pred", "aug_type", "correct", "input_tokens", "output_tokens", "prompt", "output"]].to_csv(out_csv, index=False)
